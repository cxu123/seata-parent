...
package com.luck.seata.order.service;
...
import com.luck.seata.order.repository.dao.AssetRepository;
...
public class AssetServiceImpl implements AssetService {
    ...
    @Override
    public int increase() {
        LOGGER.info("Asset Service Begin ... xid: ");
        Asset asset = assetRepository.findById(ASSET_ID).get();
        asset.setAmount(asset.getAmount().add(new BigDecimal("1")));
        assetRepository.save(asset);
        int s = no.addAndGet(1);
        LOGGER.info("no={}", s);
        if (s % 2 == 0) {
            throw new RuntimeException(
                "test exception for seata, your transaction should be rollbacked,asset=" + asset);
        } else {
            return 1;
        }
        // return 1;
